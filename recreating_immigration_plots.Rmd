...
list(
    ...
    msummary(output='markdown')
```
From here, we see that while neither of them are exactly identical, the one using the adjusted values is much closer to what they got. But the non-adjusted version actually goes against it, with a greater number of searches with _less_ crime coverage.
