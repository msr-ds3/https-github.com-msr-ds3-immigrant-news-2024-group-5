...
```{r}
list(
    early_split = lm(search_adj ~ date + arbitrary_before, discontinuity_data),
    election = lm(search_adj ~ date + after_election, discontinuity_data),
    inauguration = lm(search_adj ~ date + after_inauguration, discontinuity_data),
    late_split = lm(search_adj ~ date + arbitrary_after, discontinuity_data)
) |>
    msummary(output="markdown")
```
